...
describe("Connection Logic Visuals", () => {
  ...
  it("shows room-to-room connection for Architect", () => {
    ...
    // Architect (4,4) is above Garrison (4,5) -> direction "bottom" for (4,4)
    // Garrison (4,5) is below Architect (4,4) -> direction "top" for (4,5)

    const architectCell = screen.getByTestId("cell-4-4");
    const garrisonCell = screen.getByTestId("cell-4-5");

    // Check for connection images
    // The images are rendered inside .cell-content
    const r2r_bot = architectCell.querySelector(
  ...
  it("shows room-to-room connection for UpgradedBy rooms", () => {
    ...
    fireEvent.click(screen.getByTestId("cell-4-3")); // Top of Garrison
    ...
    const r2r_bot = commanderCell.querySelector(
      'img[src*="roomconnectroomvertical"]',
    );
  ...
  it("shows room-to-path connection when NOT connected to a path", () => {
    ...
    // (4,3) is Top relative to (4,4).
    // path2 at (4,3) does NOT have bottom connection.
    ...
    const r2p_top = garrisonCell.querySelector('img[src*="roomconnectup"]');
    expect(r2p_top).toBeTruthy();
    expect(r2p_top?.className).toContain("room-connect-top");
  });
  ...
  it("shows powered connections when powered", () => {
    ...
    // Place Architect at 4,5 (Bottom)
    fireEvent.click(screen.getByTitle("Architect's Chamber"));
    fireEvent.click(screen.getByTestId("cell-4-5"));

    // Generator at 4,4 powers Architect at 4,5
    // 4,5 should have roomToRoom connection to 4,4 (top) and it should be powered.
    ...
    expect(r2r_powered?.className).toContain("room-connect-top");
  ...
  it("shows connection to Entry point (4,0)", () => {
    ...
    // Place a room at 4,1.
    // Entry is at 4,0. 4,0 is Top relative to 4,1.
    // 4,0 is fixed as a pathfourway.
    // So 4,1 should NOT have a connection to 4,0 because 4,0 HAS a bottom connection.
    ...
    const r2p_top = garrisonCell.querySelector('img[src*="roomconnectup"]');
    expect(r2p_top).toBeFalsy();

    // If we place a path next to Entry that DOES NOT connect to it...
    // Actually the logic for 4,0 in App.tsx is:
    // } else if ((neighborCell?.type === "path" || (nx === 4 && ny === 0)) && cell.roomId !== "Generator") {
    ...
    //   }
    // }
    // pathfourway has ALL connections, so !pathConns[...] will be false.
    // To see a connection to 4,0 we'd need it to NOT have a connection in that direction, but it's hardcoded as fourway.
  });
  ...
  it("shows connection to Atziri's Chamber (4,9)", () => {
    ...
    // 4,9 is Top relative to 4,8 (since y+1 is top).
    fireEvent.click(screen.getByTitle("Garrison"));
